...
# >>> TEMPORARY ENDPOINT — DELETE AFTER DOWNLOADING ALL OUTPUTS <<<
_plan_cache = {}  # {family: plan_dict} -- avoids regenerating per day


@app.get("/api/admin/generate")
def generate_jolt(
    family: str = "revelation",
    day: int = 1,
    mixed: bool = False,
    target: str = "Exercise every day",
    charge: str = "I keep saying I'll start tomorrow and I'm tired of it",
):
    """Generate a V6 speech for a specific family+day.
    Usage:
      /api/admin/generate?family=revelation&day=1            -> raw speech MP3
      /api/admin/generate?family=revelation&day=1&mixed=true  -> mixed with music
    Families: revelation, primal, spectral, adventure
    """
    import sys
    import tempfile
    import subprocess

    # Make jolt_prompts_V6 importable by make_protocol
    svc = str(Path(__file__).resolve().parent / "services")
    if svc not in sys.path:
        sys.path.insert(0, svc)
    import make_protocol as mp
    ...
    valid_families = ["revelation", "primal", "spectral", "adventure"]
    if family not in valid_families:
        raise HTTPException(400, f"family must be one of {valid_families}")

    # Override make_protocol constants
    mp.WPS = 1.80
    ...
    print(f"[generate] family={family} day={day} mixed={mixed} "
          f"track={track_id} words={track['words']}")

    # Generate or reuse plan
    cache_key = f"{family}:{target}:{charge}"
    if cache_key not in _plan_cache:
        plan_user = jp.build_plan_prompt(target, charge)
        plan = jp.parse_plan(mp.call(jp.PLAN_SYSTEM, plan_user, temperature=0.4))
        if "error" in plan:
            raise HTTPException(400, f"plan refused: {plan['error']}")
        _plan_cache[cache_key] = plan
        print(f"[generate] plan created and cached for {cache_key}")
    else:
        print(f"[generate] reusing cached plan for {cache_key}")
    plan = _plan_cache[cache_key]

    # Generate speech
    if day == 1:
        user_prompt = jp.build_speech1_prompt(
            target, charge, plan["days"][0]["action"], track["words"],
            axes=plan["axes"], sections=track["sections"],
            lead_in=track["lead_in"])
        speech = mp.call(jp.SPEECH1_SYSTEM, user_prompt)
    else:
        completed = [d["action"] for d in plan["days"] if d["day"] < day]
        system, user_prompt = jp.build_speechn_prompts(
            day, plan, target, charge, completed, track["words"],
            sections=track["sections"], lead_in=track["lead_in"])
        speech = mp.call(system, user_prompt)

    if speech.strip().startswith("REWIRE_UNSAFE"):
        raise HTTPException(400, "speech flagged as unsafe")

    print(f"[generate] speech generated, length={len(speech.split())} words")

    # TTS
    voice_cfg = cfg.PROTOCOL_VOICE.get("activate", cfg.PROTOCOL_VOICE["activate"])
    wav_path = synth(speech, voice_cfg["voice_id"], cfg.ELEVENLABS_API_KEY,
                     voice_settings=voice_cfg["voice_settings"])
    ...
    if not mixed:
        # Return raw speech MP3
        try:
            data = Path(mp3_tmp.name).read_bytes()
            fname = f"{family}_day{day}_speech.mp3"
            return Response(
        ...
        music_path = cfg.ASSETS_DIR / "music" / track["file"]
        if not music_path.exists():
            raise HTTPException(404, f"music file not found: {track['file']}")

        mix_tmp = tempfile.NamedTemporaryFile(suffix=".mp3", delete=False)
        mix_tmp.close()
        try:
            do_mix(
                voice_path=mp3_tmp.name,
                music_path=str(music_path),
                out_path=mix_tmp.name,
                content_duration_sec=track["duration"],
                ffmpeg_bin=cfg.FFMPEG_BIN,
            )
            data = Path(mix_tmp.name).read_bytes()
            fname = f"{family}_day{day}_mixed.mp3"
            return Response(
                content=data, media_type="audio/mpeg",
                headers={
                    "Content-Disposition": f"attachment; filename={fname}",
            ...
            for f in [wav_path, mp3_tmp.name, mix_tmp.name]:
                try:
                    os.unlink(f)
                except OSError:
                    pass
